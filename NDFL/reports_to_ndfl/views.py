...
def _calculate_additional_commissions(request, user, target_report_year, target_year_files):
    global _processing_had_error
    dividend_commissions = defaultdict(Decimal)
    other_commissions_details = defaultdict(lambda: defaultdict(Decimal))
    
    sell_trade_ids = set()
    logger.info(f"COMMISSION_DEBUG: _calculate_additional_commissions called for year {target_report_year}.")
    logger.info(f"COMMISSION_DEBUG: target_year_files contains {target_year_files.count()} file(s).")
    ...
    if not target_year_files.exists():
        ...
        logger.info(f"COMMISSION_DEBUG: No files for target year {target_report_year}. Returning empty commissions.")
        return dividend_commissions, other_commissions_details

    logger.info(f"COMMISSION_DEBUG: Starting Pass 1 to collect sell trade IDs.")
    for file_instance in target_year_files:
        logger.info(f"COMMISSION_DEBUG: Pass 1 - Processing file: {file_instance.original_filename}")
        try:
            with file_instance.xml_file.open('rb') as xml_file_content_stream:
                content_bytes = xml_file_content_stream.read()
                xml_string = ""
                ...
                if not xml_string:
                    logger.warning(f"COMMISSION_DEBUG: Pass 1 - XML string is empty for file {file_instance.original_filename}")
                    continue
                
                root = ET.fromstring(xml_string)
                trades_element = root.find('.//trades')
                if trades_element:
                    detailed_trades = trades_element.find('detailed')
                    if detailed_trades:
                        for trade_node in detailed_trades.findall('node'):
                            operation = trade_node.findtext('operation', '').strip().lower()
                            if operation == 'sell':
                                trade_id = trade_node.findtext('trade_id')
                                if trade_id:
                                    sell_trade_ids.add(trade_id.strip())
                                    logger.info(f"COMMISSION_DEBUG: Pass 1 - Added sell trade ID: {trade_id.strip()} from {file_instance.original_filename}")
        except Exception as e:
            _processing_had_error[0] = True 
            logger.error(f"COMMISSION_DEBUG: Ошибка при сборе ID сделок продажи из {file_instance.original_filename} для комиссий: {e}", exc_info=True)
            messages.error(request, f"Ошибка при обработке файла {file_instance.original_filename} для сбора ID сделок продажи.")
    logger.info(f"COMMISSION_DEBUG: Pass 1 - Collected sell_trade_ids: {sell_trade_ids}")

    logger.info(f"COMMISSION_DEBUG: Starting Pass 2 to process commissions.")
    for file_instance in target_year_files:
        logger.info(f"COMMISSION_DEBUG: Pass 2 - Processing file: {file_instance.original_filename}")
        try:
            with file_instance.xml_file.open('rb') as xml_file_content_stream:
                content_bytes = xml_file_content_stream.read()
                xml_string = ""
                ...
                if not xml_string:
                    logger.warning(f"COMMISSION_DEBUG: Pass 2 - XML string is empty for file {file_instance.original_filename}")
                    continue
                
                root = ET.fromstring(xml_string)

                cash_in_outs_element = root.find('.//cash_in_outs')
                logger.info(f"COMMISSION_DEBUG: File {file_instance.original_filename} - cash_in_outs_element found: {cash_in_outs_element is not None}")
                if cash_in_outs_element:
                    for node_cio in cash_in_outs_element.findall('node'):
                        cio_type = node_cio.findtext('type', '').strip().lower()
                        payment_date_str_cio = node_cio.findtext('pay_d', node_cio.findtext('datetime', ''))
                        cio_comment_original = node_cio.findtext('comment', '')
                        logger.info(f"COMMISSION_DEBUG: CIO Node: type='{cio_type}', date_str='{payment_date_str_cio}', comment='{cio_comment_original[:60]}...'")
                        
                        payment_date_obj_cio = None
                        if payment_date_str_cio:
                            try:
                                dt_part_cio = payment_date_str_cio.split(' ')[0]
                                payment_date_obj_cio = datetime.strptime(dt_part_cio, '%Y-%m-%d').date()
                            except ValueError:
                                logger.warning(f"COMMISSION_DEBUG: Некорректная дата '{payment_date_str_cio}' для CIO ID {node_cio.findtext('id', 'N/A')}")
                                continue 
                        
                        if payment_date_obj_cio and payment_date_obj_cio.year == target_report_year:
                            logger.info(f"COMMISSION_DEBUG: CIO Node date matches target year {target_report_year}.")
                            if cio_type == 'agent_fee':
                                cio_comment_lower = cio_comment_original.strip().lower()
                                logger.info(f"COMMISSION_DEBUG: CIO Node is agent_fee. Lower comment: '{cio_comment_lower[:60]}...'")
                                if "дивиденд" in cio_comment_lower: 
                                    amount_val_str = node_cio.findtext('amount', '0')
                                    amount_val = _str_to_decimal_safe(amount_val_str, 'dividend agent_fee amount', node_cio.findtext('id', 'N/A_CIO_AGENT_FEE'))
                                    currency_cio = node_cio.findtext('currency', 'RUB').strip().upper()
                                    if currency_cio:
                                        dividend_commissions[currency_cio] += abs(amount_val)
                                        logger.info(f"COMMISSION_DEBUG: Adding to dividend_commissions: Currency={currency_cio}, Amount={abs(amount_val)}. Total now: {dividend_commissions[currency_cio]}")
                                    else:
                                        logger.warning(f"COMMISSION_DEBUG: CIO agent_fee has no currency. ID: {node_cio.findtext('id', 'N/A_CIO_AGENT_FEE')}")
                        
                commissions_main_element = root.find('.//commissions') 
                logger.info(f"COMMISSION_DEBUG: File {file_instance.original_filename} - commissions_main_element found: {commissions_main_element is not None}")
                if commissions_main_element:
                    ...
                    if detailed_comm:
                        for comm_node in detailed_comm.findall('node'):
                            ...
                            sum_val = _str_to_decimal_safe(sum_str, 'commission sum', f"type: {comm_node.findtext('type', 'N/A_COMM_TYPE')}, file: {file_instance.original_filename}")
                            ...
                            logger.info(f"COMMISSION_DEBUG: Comm Node: type='{comm_type_str}', sum='{sum_str}', currency='{currency}', datetime='{comm_datetime_str}', comment='{comm_comment[:60]}...'")
                            ...
                            if comm_datetime_str:
                                try:
                                    ...
                                    logger.warning(f"COMMISSION_DEBUG: Некорректная дата комиссии '{comm_datetime_str}' для типа '{comm_type_str}' в файле {file_instance.original_filename}")
                                    continue 
                            ...
                            if not currency:
                                logger.warning(f"COMMISSION_DEBUG: Пропуск комиссии без валюты: Тип '{comm_type_str}', Сумма '{sum_str}' в файле {file_instance.original_filename}")
                                continue
                            
                            if sum_val == Decimal(0):
                                continue

                            is_sell_trade_commission = False
                            category_key = None

                            if comm_type_str.startswith("За сделку: "):
                                trade_id_match = re.search(r"За сделку: (\S+)", comm_type_str)
                                if trade_id_match:
                                    trade_id = trade_id_match.group(1).strip()
                                    if trade_id in sell_trade_ids:
                                        is_sell_trade_commission = True
                                    else: 
                                        category_key = "Комиссии за покупку ценных бумаг"
                                else: 
                                    category_key = f"Комиссии 'За сделку' (ID не найден): {comm_type_str}"
                                    logger.warning(f"COMMISSION_DEBUG: Не удалось извлечь trade_id из типа комиссии: {comm_type_str} в {file_instance.original_filename}")
                            
                            elif comm_type_str.startswith("Проценты за использование денежных средств"):
                            ...
                            if not is_sell_trade_commission and category_key: 
                                other_commissions_details[category_key][currency] += sum_val
                                logger.info(f"COMMISSION_DEBUG: Adding to other_commissions: Category='{category_key}', Currency='{currency}', Amount='{sum_val}'. Total now for this cat/curr: {other_commissions_details[category_key][currency]}")
        
        except ET.ParseError as e_parse:
            _processing_had_error[0] = True
            logger.error(f"COMMISSION_DEBUG: Ошибка парсинга XML в {file_instance.original_filename} при расчете комиссий: {e_parse}", exc_info=True)
            ...
            _processing_had_error[0] = True
            logger.error(f"COMMISSION_DEBUG: Неожиданная ошибка при обработке файла {file_instance.original_filename} для комиссий: {e}", exc_info=True)
    ...
    logger.info(f"COMMISSION_DEBUG: Finished _calculate_additional_commissions. Returning dividend_commissions: {json.dumps(dict(dividend_commissions), default=str)}")
    logger.info(f"COMMISSION_DEBUG: Returning other_commissions_details: {json.dumps({k: dict(v) for k, v in other_commissions_details.items()}, default=str)}")
    return dividend_commissions, other_commissions_details
...
def _process_and_get_trade_data(request, user, target_report_year):
    ...
    logger.info(f"PROCESS_DEBUG: Starting _process_and_get_trade_data for year {target_report_year} for user {user.username}")
    ...
    if not relevant_files_for_history.exists():
        ...
        logger.info(f"PROCESS_DEBUG: No relevant files for history (year <= {target_report_year}). Returning empty data.")
        return {}, [], Decimal(0), Decimal(0), _processing_had_error[0], defaultdict(Decimal), defaultdict(lambda: defaultdict(Decimal))

    logger.info(f"PROCESS_DEBUG: Found {relevant_files_for_history.count()} files for history (year <= {target_report_year}).")
            ...
            with first_file_instance.xml_file.open('rb') as xml_file_content_stream:
                ...
                if xml_string_temp:
                    ...
                    if date_start_el_temp is not None and date_start_el_temp.text:
                        ...
                        logger.info(f"PROCESS_DEBUG: Earliest report start datetime set to {earliest_report_start_datetime} from file {first_file_instance.original_filename}")
        except Exception as e_early_date:
            logger.error(f"PROCESS_DEBUG: Не удалось определить самую раннюю дату начала отчета из {first_file_instance.original_filename}: {e_early_date}")
            _processing_had_error[0] = True
    ...
    for file_instance in relevant_files_for_history:
        logger.info(f"PROCESS_DEBUG: Processing file for main data: {file_instance.original_filename} (Year: {file_instance.year})")
        ...
        try:
            with file_instance.xml_file.open('rb') as xml_file_content_stream:
                ...
                if not xml_string_loop: 
                    logger.warning(f"PROCESS_DEBUG: XML string is empty for file {file_instance.original_filename}. Skipping.")
                    continue
                root = ET.fromstring(xml_string_loop)
                ...
                if current_file_date_start_str:
                    ...
                    except ValueError: logger.warning(f"PROCESS_DEBUG: Некорректная date_start {current_file_date_start_str} в {file_instance.original_filename}")
                ...
                if earliest_report_start_datetime and current_file_start_dt == earliest_report_start_datetime and file_instance.id not in processed_initial_holdings_file_ids:
                    logger.info(f"PROCESS_DEBUG: Processing initial holdings for file {file_instance.original_filename}")
                        ...
                        if positions_el:
                            for pos_node in positions_el.findall('node'):
                                try:
                                    ...
                                    if not isin: instr_nm_log = pos_node.findtext('name', 'N/A').strip(); logger.info(f"PROCESS_DEBUG: Пропуск НО в {file_instance.original_filename} для '{instr_nm_log}': отсутствует ISIN."); continue
                                    ...
                                    logger.info(f"PROCESS_DEBUG: Added initial holding: {op_details_dict_for_ref}")
                                except (AttributeError, ValueError) as e_init:
                                     _processing_had_error[0] = True; logger.error(f"PROCESS_DEBUG: Ошибка парсинга НО в {file_instance.original_filename}: {e_init}", exc_info=True)
                    ...
                    if detailed_element:
                        for node_element in detailed_element.findall('node'):
                            ...
                            try:
                                ...
                                if not current_isin: logger.warning(f"PROCESS_DEBUG: Пропуск сделки (ID: {current_trade_id_for_log}): нет ISIN/ISSUE_NB."); _processing_had_error[0] = True; continue
                                ...
                                if currency_code != 'RUB' and rate_decimal is None: _processing_had_error[0] = True; logger.error(f"PROCESS_DEBUG: Пропуск сделки {current_trade_id_for_log} в FIFO (нет курса {currency_code})."); continue
                                ...
                                _processing_had_error[0] = True; logger.error(f"PROCESS_DEBUG: Ошибка обработки узла сделки (ID: {current_trade_id_for_log}) в {file_instance.original_filename}: {e_node}", exc_info=True)
                ...
                if is_target_year_file_for_dividends:
                    logger.info(f"PROCESS_DEBUG: Processing dividends for target year file {file_instance.original_filename}")
                    cash_in_outs_element = root.find('.//cash_in_outs')
                    if cash_in_outs_element:
                        for node_cio in cash_in_outs_element.findall('node'):
                            try:
                                ...
                                if cio_type == 'dividend':
                                    ...
                                    if payment_date_str:
                                        ...
                                        except ValueError: logger.warning(f"PROCESS_DEBUG: Некорректная дата выплаты дивиденда '{payment_date_str}' (ID {cio_id_for_log})"); continue
                                    ...
                                    logger.info(f"PROCESS_DEBUG: Processed dividend event: {dividend_events_in_current_file[div_event_key]}")
                            except Exception as e_div_pre_parse: logger.error(f"PROCESS_DEBUG: Ошибка предварительного парсинга дивиденда (ID: {cio_id_for_log}) в {file_instance.original_filename}: {e_div_pre_parse}", exc_info=True)
                            ...
                            try:
                                ...
                                if cio_type == 'tax' and ("налог за корпоративное действие" in cio_comment.lower() or "tax for corporate action" in cio_comment.lower()):
                                    ...
                                    if target_dividend_event: 
                                        ...
                                        logger.info(f"PROCESS_DEBUG: Added tax {abs(tax_amount_val)} to dividend event {target_dividend_event.get('corporate_action_id')}")
                                    else: logger.warning(f"PROCESS_DEBUG: Не удалось связать налог (ID {cio_id_for_log}, CA_ID: {ca_id_from_details_cio}, Дата: {tax_date_obj}) с дивидендом в файле {file_instance.original_filename}.")
                            except Exception as e_tax_parse: logger.error(f"PROCESS_DEBUG: Ошибка парсинга узла налога (ID: {cio_id_for_log}) в {file_instance.original_filename}: {e_tax_parse}", exc_info=True)

                    all_dividend_events_final_list.extend(dividend_events_in_current_file.values())

        except ET.ParseError: _processing_had_error[0] = True; logger.warning(f"PROCESS_DEBUG: Ошибка парсинга XML: {file_instance.original_filename}", exc_info=True); messages.error(request, f"Ошибка парсинга XML в файле {file_instance.original_filename}.")
        except Exception as e: _processing_had_error[0] = True; logger.error(f"PROCESS_DEBUG: Ошибка обработки файла {file_instance.original_filename}: {e}", exc_info=True); messages.error(request, f"Неожиданная ошибка при обработке файла {file_instance.original_filename}.")
    ...
    if trade_and_holding_ops:
        logger.info(f"PROCESS_DEBUG: Starting FIFO processing for {len(trade_and_holding_ops)} operations.")
        ...
        logger.info(f"PROCESS_DEBUG: FIFO processing finished.")
    ...
    while loop_idx < len(all_display_events):
        ...
        if display_type == 'trade' and details and current_event_wrapper.get('datetime_obj'):
            ...
            if len(trades_to_potentially_aggregate) > 1:
                ...
                if combined_details.get('operation','').lower() == 'sell':
                    combined_details['fifo_cost_rub_str'] = f"{total_fifo_cost_rub:.2f}"
                elif combined_details.get('operation','').lower() == 'buy':
    ...
    logger.info(f"PROCESS_DEBUG: files_for_sales_scan for commission calculation (year {target_report_year}): {[f.original_filename for f in files_for_sales_scan]}")
    if files_for_sales_scan.exists():
        for file_instance_scan in files_for_sales_scan:
            ...
            except Exception as e_sales_scan: _processing_had_error[0] = True; logger.error(f"PROCESS_DEBUG: Ошибка сканирования продаж: {e_sales_scan}", exc_info=True)
    ...
    for event_data_wrapper in all_display_events:
        ...
        if not current_event_isin:
            ...
            logger.warning(f"PROCESS_DEBUG: Пропуск события без основного ISIN при финальной группировке: {log_id}"); continue
                ...
                if event_wrapper.get('display_type') == 'trade':
                    ...
                    if details and details.get('operation','').lower() == 'sell':
                        ...
                        if event_datetime_obj and event_datetime_obj.year == target_report_year:
                            ...
                            commission_curr = details.get('commission', Decimal(0)) 
                            ...
                            if currency_code != 'RUB':
                                ...
                                if match_rate:
                                    try:
                                        cbr_rate_for_sale = Decimal(match_rate.group(1))
                                    except InvalidOperation:
                                        logger.error(f"PROCESS_DEBUG: Failed to parse CBR rate: '{rate_str_from_event}' for sale ID {details.get('trade_id')}. Using 0 for non-RUB.")
                                        if currency_code != 'RUB': cbr_rate_for_sale = Decimal(0)
                                elif currency_code != 'RUB':
                                    logger.error(f"PROCESS_DEBUG: CBR rate string not found or unmatchable: '{rate_str_from_event}' for currency {currency_code}, sale ID {details.get('trade_id')}. Using 0.")
                                    cbr_rate_for_sale = Decimal(0)

                            sale_amount_curr = _str_to_decimal_safe(sale_amount_curr, 'sale_amount_for_total_profit_calc', details.get('trade_id'))
                            ...
                            if total_expenses_for_sale_rub is None: total_expenses_for_sale_rub = Decimal(0)
                                ...
                                total_expenses_for_sale_rub = _str_to_decimal_safe(total_expenses_for_sale_rub, 'total_expenses_for_profit_calc', details.get('trade_id'))
    ...
    dividend_commissions_details, other_commissions_details = _calculate_additional_commissions(request, user, target_report_year, files_for_sales_scan)
    
    logger.info(f"PROCESS_DEBUG: Finished _process_and_get_trade_data for year {target_report_year}.")
    if _processing_had_error[0]:
        logger.warning(f"PROCESS_DEBUG: Processing for year {target_report_year} had one or more errors.")
        
    return final_instrument_event_history, all_dividend_events_final_list, total_dividends_rub_for_year, total_sales_profit_rub_for_year, _processing_had_error[0], dividend_commissions_details, other_commissions_details
...
def upload_xml_file(request):
    ...
    if request.method == 'POST':
        ...
        if year_to_process is not None:
            ...
            dividend_commissions_data, other_commissions_data = _process_and_get_trade_data(request, user, year_to_process)

            # ДОБАВЛЕНО: Явное преобразование defaultdict в обычные dict
            if isinstance(dividend_commissions_data, defaultdict):
                dividend_commissions_data = dict(dividend_commissions_data)
            ...
            if isinstance(other_commissions_data, defaultdict):
                ...
                for category, currencies_dict in other_commissions_data.items():
                    if isinstance(currencies_dict, defaultdict):
                        converted_other_commissions[category] = dict(currencies_dict)
                    else:
                        converted_other_commissions[category] = currencies_dict
            ...
            logger.info(f"UPLOAD_VIEW: Dividend Commissions data for template: {json.dumps(dict(dividend_commissions_data), default=str)}")
            logger.info(f"UPLOAD_VIEW: Other Commissions data for template: {json.dumps({k: dict(v) for k, v in other_commissions_data.items()}, default=str)}")
